...
#define WIFI_SSID "tp-link" // change your WiFi name
#define WIFI_PASSWORD "anhvu6868"  //change your Password  WiFi


const char *server = "api.thingspeak.com";
unsigned long myChannelNumber = 1252697;
const char * myWriteAPIKey = "75IXKLW3DUC9T2OY";
...
//  lcd.begin(); // Nếu sử dụng win 10 thì dùng khai báo này
  ...
  if ( process_control <15)
  {
		...
		Firebase.begin("humi-temp-lehieu.firebaseio.com");
...
void cali_rain_ss()
{
  if(digitalRead(BT2) == 0)
  {
    lcd.clear();
    lcd.setCursor(0,0);
    lcd.print("Hieu chinh       "); 
    lcd.setCursor(0,1);
    lcd.print("cam bien mua      ");  
    delay(2000);
    lcd.setCursor(0,0);
    lcd.print("Lau kho ss truoc      "); 
    lcd.setCursor(0,1);
    lcd.print("khi hieu chinh         ");   
    delay(2000);
    
    rain_value = 0;
    for (int i =0 ; i++;i<10)
    {
      rain_value= rain_value + analogRead(Rain_Sensor);
    }
    rain_value = rain_value/10;
    rain_value = analogRead(Rain_Sensor) - 40;
    Serial.print("Hieu chinh thanh cong, nguong cam bien mua : ");Serial.println (rain_value);
    EEPROM.write(1, rain_value/4);    
    EEPROM.commit();   
    lcd.setCursor(0,0);
    lcd.print("Hieu chinh           "); 
    lcd.setCursor(0,1);
  ...
  {   
    rain_value = EEPROM.read(1)*4;
    if(rain_value < 100 || rain_value > 1000)
    {
      rain_value = 300;
    }
    Serial.print("Doc nguong cam bien mua thanh cong, nguong cam bien mua : ");Serial.println (rain_value);
  }
}
